...
| 经历 -> 简历材料 | 今天先整理一段真实经历 |
| JD -> 投递前最小修改 | 今天先完成一个投递前行动：确需修改时做 1-2 条；缺来源时补 1 条真实事实后重新判断；严格无需修改时确认并保存当前版本 |
| 投递记录 -> 轻复盘 | 今天先补齐最近 2-3 条投递记录，并选择 1 条先复盘 |

JD 路线始终只显示一个主行动和 0-2 个相关修改。严格无需修改为 0 个修改，但服务端必须先检查前 3-5 条关键要求，确认每个保留判断都有来源、没有未支撑的关键要求、没有夸大或事实错误。页面不得展示内部裁决名、关系类型或质量门术语。
...
- 修改前材料片段。
- 修改后材料片段。
- 本次做了哪 0-2 条最小修改；0 条时保存当前版本确认依据。
- 是否已投递。
- 如果已投递，记录投递时间。
- 后续要观察的真实反馈。
