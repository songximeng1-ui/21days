...
- 对照用户材料，指出当前能支撑的部分。
- 指出当前材料里还看不出来的部分。
- 服务端完成来源校验后，在确需修改时给出 1-2 条投递前最小修改动作；严格无需修改时允许 0 条。
- 生成今日行动。
...
#### JD 路线三种闭环（2026-08-05 治理修订）

以下英文状态名仅用于服务端裁决和测试，不得作为页面标签或质量门术语展示给用户：

1. `collect_evidence`：来源不足时，不生成可直接粘贴的假设句；只给 1 个补事实行动。用户补充并确认真实来源后保存，再重新提交并重新判断。
2. 实际修改：只有候选句全部可由已提供或已确认的原文支撑时，才给 1 个行动和 1-2 个相关修改。用户确认修改后保存材料版本，并记录是否投递与待观察反馈。
3. `all-keep`：允许 0 个修改，但服务端必须先检查前 3-5 条关键要求，确认每个保留判断都有来源、没有未支撑的关键要求，也没有夸大或事实错误。用户仍需执行 1 个行动：确认并保存当前版本，记录已投递状态和观察点。

三条路径都只产生一个用户可执行行动；页面只呈现普通语言的行动、依据和记录入口，不展示关系类型、内部裁决名或质量门术语。
