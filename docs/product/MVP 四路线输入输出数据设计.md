...
| 投递前最小修改 | 0-2 条；确需修改时 1-2 条，严格无需修改时允许 0 条；必须基于真实材料 |
| 投递后记录什么 | 材料版本、投递时间、反馈状态 |
| 今日行动 | 做 1-2 条有来源的最小修改、补 1 条真实事实后重新判断，或确认并保存严格无需修改的当前版本 |

JD 路线必须在服务端完成三种互斥裁决，但不得把内部裁决名展示给用户：

1. 缺来源：禁止生成可直接粘贴的假设句；闭环为“补 1 条真实事实 -> 用户确认 -> 保存补充来源 -> 重新提交并重新判断”。
2. 实际修改：闭环为“给 1 个主行动和 1-2 个相关修改 -> 用户确认 -> 保存材料版本 -> 记录是否投递与观察点”。
3. 严格无需修改：允许 0 个修改；服务端先检查前 3-5 条关键要求，确认每个保留判断都有来源、没有未支撑的关键要求、没有夸大或事实错误；闭环为“确认当前版本 -> 保存 -> 记录是否投递与观察点”。
...
| after_snippet | 修改后片段 |
| minimal_changes | 本次 0-2 条最小修改动作；0 条仅表示已通过严格无需修改检查 |
| submitted | 是否投递 |
| submitted_at | 如果已投递，投递时间 |
| observation_point | 后续要观察的真实反馈 |
