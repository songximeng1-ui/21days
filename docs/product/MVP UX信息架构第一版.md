...
flowchart TD
    ...
    E3 --> F3{"服务端来源校验"}
    F3 -->|缺来源| I3["今日行动：补 1 条真实事实后重新判断"]
    F3 -->|确需修改| J3["今日行动：做 1-2 条投递前最小修改"]
    F3 -->|严格无需修改| K3["今日行动：确认并保存当前版本"]
    I3 --> G3["记录：补充来源、重新提交结果"]
    J3 --> G3["记录：修改前后片段、JD、是否投递、投递时间"]
    K3 --> G3["记录：当前版本、是否投递、观察点"]
...
严格无需修改只在服务端检查前 3-5 条关键要求、每个保留判断都有来源、没有未支撑的关键要求、没有夸大或事实错误后成立。上述分支名属于内部状态，页面不得直接展示。
